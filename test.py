...
def test(args, TestImgLoader, model, model_fn_test, save_path, compute_metrics):
    ...
    for batch_idx, data in enumerate(tbar):
        ...
        if args.EVALUATION_METRIC:
            logging.info(
                '%s: LPIPS is %.4f, PSNR is %.4f and SSIM is %.4f' % (number[0], cur_lpips, cur_psnr, cur_ssim))
        ...
        if args.EVALUATION_METRIC:
            desc = 'Test: Avg. LPIPS = %.4f, Avg. PSNR = %.4f and SSIM = %.4f' % (
    ...
    if args.EVALUATION_METRIC:
        logging.warning(
            'Avg. LPIPS is %.4f, PSNR is %.4f and SSIM is %.4f' % (avg_val_lpips, avg_val_psnr, avg_val_ssim))
    if args.EVALUATION_TIME:
